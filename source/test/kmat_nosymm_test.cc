...
int main(int argc, char *argv[])
{
  ...
  normalMatrix D_nosymm;
  D_nosymm.resetSizesAndBlocks(matrix_size_block_info,
			       matrix_size_block_info);
  normalMatrix K_nosymm;
  K_nosymm.resetSizesAndBlocks(matrix_size_block_info,
			       matrix_size_block_info);
...

  {
    ...
    for(int i = 0; i < n-1; i++) {
      int j = i + 1;
      ergo_real value1 = 0.2 - 0.1 * ((double)rand() / RAND_MAX);
      rowind[count] = i;
      colind[count] = j;
      values[count] = value1;
      count++;
      ergo_real value2 = 0.2 - 0.1 * ((double)rand() / RAND_MAX);
      rowind[count] = j;
      colind[count] = i;
      values[count] = value2;
      count++;
    }
    D_nosymm.assign_from_sparse(rowind,
				colind,
				values,
				permutationHML,
				permutationHML);
  ...
  if(compute_K_by_boxes_sparse_nosymm(bis, integralInfo, CAM_params_not_used, J_K_params, K_nosymm, D_nosymm,
				      permutationHML,
				      inversePermutationHML) != 0)
    {
      printf("Error in compute_K_by_boxes_sparse_nosymm\n");
      return -1;
    }
  ...
  ergo_real maxabsdiff_explicitcheck = 0;
  {
    ...
    K_nosymm.get_all_values(rowind_K,
			    colind_K,
			    values_K,
			    inversePermutationHML,
			    inversePermutationHML);
	...
	ergo_real absdiff = template_blas_fabs(K_value_explicit - K_value_to_check);
  ...
  printf("K nosymm test OK, maxabsdiff_explicitcheck = %7.4g\n", (double)maxabsdiff_explicitcheck);
  return 0;
}

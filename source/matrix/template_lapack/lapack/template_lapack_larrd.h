...
#ifndef TEMPLATE_LAPACK_LARRD_HEADER
#define TEMPLATE_LAPACK_LARRD_HEADER

template<class Treal>
int template_lapack_larrd(const char *range, const char *order, const integer *n, Treal 
	*vl, Treal *vu, integer *il, integer *iu, Treal *gers, 
	Treal *reltol, Treal *d__, Treal *e, Treal *e2, 
	Treal *pivmin, integer *nsplit, integer *isplit, integer *m, 
	Treal *w, Treal *werr, Treal *wl, Treal *wu, 
	integer *iblock, integer *indexw, Treal *work, integer *iwork, 
	integer *info)
{
...
/*     Check for Errors */
    ...
    if (irange <= 0) {
	*info = -1;
    } else if (! (template_blas_lsame(order, "B") || template_blas_lsame(order, 
	    "E"))) {
	*info = -2;
    } else if (*n < 0) {
	*info = -3;
...
/*     Get machine constants */
    ...
    uflow = template_lapack_lamch("U",(Treal)0);
...
/*     if only scalar is to be done. */
    nb = template_lapack_ilaenv(&c__1, "DSTEBZ", " ", n, &c_n1, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
    if (nb <= 1) {
	nb = 0;
    }
...
/*        Gerschgorin bounds GL and GU is refined by DLAEBZ. */
	itmax = (integer) ((template_blas_log(tnorm + *pivmin) - template_blas_log(*pivmin)) / template_blas_log(2.)) + 
		2;
	work[*n + 1] = gl;
	work[*n + 2] = gl;
	work[*n + 3] = gu;
...
/*           Find negcount of initial interval boundaries GL and GU */
		...
		*info = iinfo;
	    ...
	    iwoff = *m - iwork[1];
/*           Compute Eigenvalues */
	    itmax = (integer) ((template_blas_log(gu - gl + *pivmin) - template_blas_log(*pivmin)) / template_blas_log(
		    2.)) + 2;
...
/*              semi length of error interval */
		tmp2 = (d__1 = work[j + *n] - work[j + in + *n], absMACRO(d__1)) * 
			.5;
		if (j > iout - iinfo) {
...
/*     If ORDER='E', sort the eigenvalues from smallest to largest */
    if (template_blas_lsame(order, "E") && *nsplit > 1) {
	i__1 = *m - 1;
	for (je = 1; je <= i__1; ++je) {
	    ie = 0;
	    tmp1 = w[je];

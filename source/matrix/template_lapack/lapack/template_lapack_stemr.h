...
#define TEMPLATE_LAPACK_STEMR_HEADER

template<class Treal>
int template_lapack_stemr(const char *jobz, const char *range, const integer *n, Treal *
	d__, Treal *e, const Treal *vl, const Treal *vu, const integer *il, 
	const integer *iu, integer *m, Treal *w, Treal *z__, const integer *ldz, 
	 const integer *nzc, integer *isuppz, logical *tryrac, Treal *work, 
...
/* Computing MIN */
	...
	if (wantz && alleig) {
	    ...
	    template_lapack_larrc("T", n, vl, vu, &d__[1], &e[1], &safmin, &nzcmin, &itmp, &
		    itmp2, info);
...
/*           WANTZ .EQ. FALSE. */
	...
	i__1 = -(*info);
	template_blas_erbla("DSTEMR", &i__1);
...
/*     Handle N = 0, 1, and 2 cases immediately */
    ...
    *m = 0;
    ...
    if (*n == 2) {
	if (! wantz) {
	    template_lapack_lae2(&d__[1], &e[1], &d__[2], &r1, &r2);
	} else if (wantz && ! zquery) {
	    template_lapack_laev2(&d__[1], &e[1], &d__[2], &r1, &r2, &cs, &sn);
	}
...
/*     RMAX threshold. */
    ...
    if (scale != 1.) {
	template_blas_scal(n, &scale, &d__[1], &c__1);
	i__1 = *n - 1;
	template_blas_scal(&i__1, &scale, &e[1], &c__1);
	tnrm *= scale;
	if (valeig) {
...
/*        Test whether the matrix warrants the more expensive relative approach. */
	template_lapack_larrr(n, &d__[1], &e[1], &iinfo);
    } else {
...
/*        Copy original diagonal, needed to guarantee relative accuracy */
	template_blas_copy(n, &d__[1], &c__1, &work[indd], &c__1);
    }
...
/*        Note: these settings do only affect the subset case and DLARRE */
	rtol1 = template_blas_sqrt(eps);
/* Computing MAX */
	d__1 = template_blas_sqrt(eps) * .005, d__2 = eps * 4.;
	rtol2 = maxMACRO(d__1,d__2);
    }
    template_lapack_larre(range, n, &wl, &wu, &iil, &iiu, &d__[1], &e[1], &work[inde2], &
...
/*        eigenvalues */
	...
	template_lapack_larrv(n, &wl, &wu, &d__[1], &e[1], &pivmin, &iwork[iinspl], m, &
...
L36:
	    ...
	    rtol2 = eps * 4.;
	    template_lapack_larrj(&in, &work[indd + ibegin - 1], &work[inde2 + ibegin - 1], 
...
/*     If matrix was scaled, then rescale eigenvalues appropriately. */
    ...
    if (scale != 1.) {
	d__1 = 1. / scale;
	template_blas_scal(m, &d__1, &w[1], &c__1);
    }
...
/*     possibly along with eigenvectors. */
    ...
    if (nsplit > 1) {
	if (! wantz) {
	    template_lapack_lasrt("I", m, &w[1], &iinfo);
	    if (iinfo != 0) {
		*info = 3;
		return 0;
	    }
...
/* L50: */
		...
		if (i__ != 0) {
		    ...
		    w[j] = tmp;
		    if (wantz) {
			template_blas_swap(n, &z__[i__ * z_dim1 + 1], &c__1, &z__[j * 
				z_dim1 + 1], &c__1);
			itmp = isuppz[(i__ << 1) - 1];

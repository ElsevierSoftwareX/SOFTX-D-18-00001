...
/** @file lebedev_laikov.cc Evaluate angular grid of requested order. 
...
** ll_npoint
**
** lvalue : grid complete through this value of angular momentum
** quantum number l.
**
...
int ll_npoint(int lvalue)
{
  ...
  /* 
     printf(" Lebedev_Laikov_npoint: lvalue > 131. "
     "No grids of this type available.\n");
  */
  return 0;
}
...
int ll_order(int npoint)
{
  ...
  for(i=0; i< sizeof(map)/sizeof(map[0]); i++) {
    if( npoint <= map[i].npoints)
      return map[i].order;
  }
  return map[sizeof(map)/sizeof(map[0])-1].order;
}


static int
ll_Oh (int n, real a, real b, real v,
       real *x, real *y, real *z, real *w)
{
  ...
  switch (n) {
    case 1:
      ...
      return 6;
    case 2:
      a = 1.0/template_blas_sqrt(static_cast<real>(2.0));
      ...
      return 12;
    case 3:
      a = 1.0/template_blas_sqrt(static_cast<real>(3.0));
      ...
      return 8;
    case 4:
      b = template_blas_sqrt(1.0 - 2.0*a*a);
    ...
    case 5:
      ...
      return 24;
    case 6:
      c = template_blas_sqrt(1.0 - a*a - b*b);
...
int
ll_sphere(int N, real *X, real *Y, real *Z, real *W)
{
#define A n+=ll_Oh(
...
static void
getangle(real x, real y, real z, real &a, real &b)
{
  real ang_x, fact;
  b=acos(z);

  fact=template_blas_sqrt(x*x+y*y);
...
int main ()
{
  ...
  static real x[MMAX], y[MMAX], z[MMAX], w[MMAX], alpha[MMAX],beta[MMAX];
  static real xn[MMAX*(NMAX+1)], yn[MMAX*(NMAX+1)], zn[MMAX*(NMAX+1)];
  static real s[NMAX+2];
  real S, S0, r, rmax;
  for (N = 1; N <= NMAX; N++) {
    M = NW[N];

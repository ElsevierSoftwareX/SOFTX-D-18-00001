...
currtestdirname=tmpdir_test_hf
...
echo Testing h2 with bond distance 5 au, HF/6-31G** with naive Fock mat
rm -f ergoscf.out
./ergo <<EOINPUT > /dev/null
molecule_inline
H   -2.5  0  0
...
EOF
basis = "6-31Gss"
J_K.use_naive_fockmat_constr = 1
run "HF"
EOINPUT
if
check_final_energy -0.8433867 1e-7
...
echo Testing cnof HF/3-21G with dense H_core matrix
rm -f ergoscf.out
./ergo <<EOINPUT > /dev/null
molecule_inline
C     0.0       0.0       0.0
...
EOF
basis = "3-21G"
scf.use_simple_dense_H_core = 1
run "HF"
EOINPUT
if
check_final_energy -262.7198779 1e-6
then
echo OK
else
echo ERROR
